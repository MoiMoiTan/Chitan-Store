    ...
    <div className="py-5 flex flex-col md:flex-row items-start gap-5">
        ...
        {categories?.map((item) => (
          <Button
            ...
            className={`bg-transparent border-0 rounded-none 
                text-darkColor shadow-none hover:bg-darkColor/80 
                hover:text-white font-semibold hoverEffect 
                border-b last:border-b-0 ${item?.slug?.current === currentSlug && "bg-darkColor text-white border-darkColor"}`}
          >
            {item?.title}
          </Button>
        ))}
      ...
      <div className="w-full">
        {loading ? (
          ...
          justify-center py-10 min-h-80 space-y-4 text-center bg-gray-100
